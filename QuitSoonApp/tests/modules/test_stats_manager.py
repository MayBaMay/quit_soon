...
class SmokeStatsTestCaseBigData(TestCase):
    ...
    def setUp(self):
        ...
        self.stats = SmokeStats(
            self.usertest,
            make_aware(datetime.datetime(2019, 11, 28, 12, 0), pytz.utc),
            -120
            )
    ...
    def test_nb_full_period_for_average(self):
        ...
        self.assertEqual(
            self.stats.nb_full_period_for_average(datetime.date(2019,11,28), 'day'),
            61
            )
        self.assertEqual(
            self.stats.nb_full_period_for_average(datetime.date(2019,11,28), 'week'),
            8
            )
        self.assertEqual(
            self.stats.nb_full_period_for_average(datetime.date(2019,11,28), 'month'),
            1
            )
        self.assertEqual(
            self.stats.nb_full_period_for_average(datetime.date(2020,8,5), 'day'),
            312
            )
        self.assertEqual(
            self.stats.nb_full_period_for_average(datetime.date(2020,8,5), 'week'),
            44
            )
        self.assertEqual(
            self.stats.nb_full_period_for_average(datetime.date(2020,8,5), 'month'),
            10
            )
    ...
    def test_nb_full_period_for_average_dates_first_and_last_day_of_month(self):
        ...
        stats = SmokeStats(
            user,
            make_aware(datetime.datetime(2019, 2, 1, 12, 0), pytz.utc),
            -120
            )
        self.assertEqual(
        ...
        self.assertEqual(
            stats.nb_full_period_for_average(datetime.date(2020, 2, 1), 'week'),
            8
            )
        self.assertEqual(
            stats.nb_full_period_for_average(datetime.date(2020, 2, 1), 'month'),
            2
            )

    def test_update_models_dt_user(self):
        """test method update_dt_user_model_field with actual timedelta tz_offset"""
        conso = ConsoCig.objects.get(
            user=self.usertest,
            datetime_cig=datetime.datetime(2019, 9, 28, 9, 0, tzinfo=pytz.utc)
            )
        self.assertEqual(
            conso.user_dt,
    ...
    def test_average_money_per_day(self):
        ...
        self.assertEqual(
            round(self.stats.average_money_per_day, 2),
            Decimal('2.61')
            )
    ...
    def test_total_money_with_starting_nb_cig(self):
        ...
        self.assertEqual(
            round(self.stats.total_money_with_starting_nb_cig, 2),
            Decimal('591.70')
            )
...
class SmokeStatsTestCaseSmallData(TestCase):
    ...
    def setUp(self):
        ...
        self.stats = SmokeStats(
            self.usertest,
            make_aware(datetime.datetime(2020, 6, 20, 12, 0), pytz.utc),
            -120
            )
...
class StatsFirstDay(TestCase):
    ...
    def setUp(self):
        ...
        ConsoCig.objects.filter(
            user=self.usertest,
            datetime_cig__gt=datetime.datetime(2020, 6, 19, 22, 0, tzinfo=pytz.utc)
            ).delete()
        self.stats = SmokeStats(
            self.usertest,
            make_aware(datetime.datetime(2020, 6, 19, 23, 59), pytz.utc),
            -120
            )
    ...
    def test_average_money_per_day(self):
        ...
        self.assertEqual(
            self.stats.average_money_per_day,
            self.stats.total_money_smoked
            )
    ...
    def test_total_money_with_starting_nb_cig(self):
        ...
        self.assertEqual(
            self.stats.total_money_with_starting_nb_cig,
            Decimal('9.700')
            )
...
class HealthyStatsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.stats = HealthyStats(
            self.usertest,
            make_aware(datetime.datetime(2019, 11, 28, 23, 59), pytz.utc),
            -120
            )

    def test_update_models_dt_user(self):
        """test method updating field dt_user in db wwith actual tz_offset"""
        conso = ConsoAlternative.objects.get(
            user=self.usertest,
            datetime_alter=datetime.datetime(2019, 9, 28, 20, 20, tzinfo=pytz.utc)
            )
        self.assertEqual(
            conso.user_dt,
            datetime.datetime(2019, 9, 28, 22, 20, tzinfo=pytz.utc)
            )

    def test_filter_queryset_for_report(self):
        """test method filter_queryset_for_report"""
        self.assertEqual(
            self.stats.filter_queryset_for_report().count(), 34
            )
        self.assertEqual(
            self.stats.filter_queryset_for_report(type_alt='Sp').count(), 16
            )
        self.assertEqual(
            self.stats.filter_queryset_for_report('Su').count(), 24
            )
        self.assertEqual(
            self.stats.filter_queryset_for_report('Su', type_alt='P').count(), 9
            )

    def test_report_substitut_per_period(self):
        """test method report_alternative_per_period """
        self.assertEqual(
            self.stats.report_alternative_per_period(
    ...
    def test_filter_by_period(self):
        ...
        self.assertEqual(
            self.stats.filter_by_period(
                datetime.date(2019, 10, 11),
                'day',
                self.stats.user_activities
                ).count(),
            2
            )
        self.assertEqual(
